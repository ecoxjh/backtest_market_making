...
def aggregate_returns(returns, convert_to):
    ...
    if convert_to == 'daily':
        return returns.groupby(
            [lambda x: x.year,
             lambda x: x.month,
             lambda x: x.day]).apply(cumulate_returns)
...
def create_cagr(equity, periods=240):
    """
...
def create_sharpe_ratio(returns, periods=240):
    """
...
def create_sortino_ratio(returns, periods=240):
    """
